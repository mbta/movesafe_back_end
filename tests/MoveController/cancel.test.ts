...
describe('POST /api/cancel-move', () => {
    ...
    it('should return 400 if parameters are missing', async () => {
        const response = await request(app).post('/api/cancel-move').query({});
    ...
    it('should return 400 if move is not found', async () => {
        ...
        const response = await request(app).post('/api/cancel-move').query({ move_id: '1' });
        expect(rollback).toHaveBeenCalled();
        expect(response.status).toBe(400);
        expect(response.body).toHaveProperty('message', 'Could not find move with given id');
    });

    it('should return 400 if move is already done', async () => {
        ...
        expect(rollback).toHaveBeenCalled();
        expect(response.status).toBe(400);
        expect(response.body).toHaveProperty('message', 'This move is already done');
    });

    it('should cancel the move successfully', async () => {
        const move = { id: '1', status: 'pending_move', save: jest.fn().mockResolvedValue({ id: '1', status: 'cancelled' }) };
        (Move.findByPk as jest.Mock).mockResolvedValue(move);
