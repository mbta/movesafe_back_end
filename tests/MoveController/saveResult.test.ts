...
describe('POST /api/move-result', () => {
    ...
    it('should save inspection answers and update move status', async () => {
        let moveMock = { id: '1', status: MoveStatus.pending_checklist, save: jest.fn() } as unknown as Move;
        (Move.findByPk as jest.Mock).mockResolvedValue(moveMock);

        const inspectionAnswersMock = { save: jest.fn() } as unknown as InspectionAnswer;
        (InspectionAnswer.build as jest.Mock).mockReturnValue(inspectionAnswersMock);
        ...
        expect(moveMock.status).toBe(MoveStatus.pending_move);
        expect(moveMock.save).toHaveBeenCalled();
