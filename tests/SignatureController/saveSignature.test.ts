...
describe('POST /api/signature', () => {
    ...
    it('should return 400 if trying to save yardmaster signature before move is done', async () => {
        ...
        expect(rollback).toHaveBeenCalled();
        expect(res.status).toBe(400);
        expect(res.body).toEqual({ message: "Can not save yardmaster signature before move done" });
    });
    ...
    it('should save the signature and return 200 with signature object', async () => {
        ...
        (s3Client.saveImage as jest.Mock).mockResolvedValue('http://s3-url.com/image.png');
        ...
        expect(s3Client.saveImage).toHaveBeenCalledWith(file.buffer, 'Test Caption', 'image/png');
