...
const getKeyCloakAuthToken = async (): Promise<string> => {
    ...
    const createTokenResponse: AxiosResponse<ICreateTokenResponse> = await axios.post(`${process.env.KEY_CLOAK_ADDRESS}/realms/${process.env.KEY_CLOAK_REALM}/protocol/openid-connect/token`,
        authData,
        {
            headers: {
...
export const create = async (req: Request, res: Response) => {
    try {
        ...
        const createUserResponse: AxiosResponse<void> = await axios.post(`${process.env.KEY_CLOAK_ADDRESS}/admin/realms/${process.env.KEY_CLOAK_REALM}/users`,
            {
        ...
        const usersResponse: any = await axios.get(`${process.env.KEY_CLOAK_ADDRESS}/admin/realms/${process.env.KEY_CLOAK_REALM}/users`,
            {
                headers: {
                    "Authorization": `Bearer ${token}`,
                },
        ...
        const keyCloakUser = usersResponse.data.find((e: any) => e.username === user.username);
        ...
        if (!usersGroupSplit?.length) {
            return res.status(400).json({ message: "Could not determine users group" });
        }
...
export const updateUser = async (req: Request, res: Response) => {
    try {
        ...
        const updateUserResponse: AxiosResponse<void> = await axios.put(`${process.env.KEY_CLOAK_ADDRESS}/admin/realms/${process.env.KEY_CLOAK_REALM}/users/${id}`,
            {
                username: user.username,
                enabled: user.enabled,
                groups: user.groups,
...
export const deleteUser = async (req: Request, res: Response) => {
    try {
        ...
        const deleteUserResponse: AxiosResponse<void> = await axios.delete(`${process.env.KEY_CLOAK_ADDRESS}/admin/realms/${process.env.KEY_CLOAK_REALM}/users/${id}`,
            {
                headers: {
                    "Authorization": `Bearer ${token}`,
                },
...
export const getAllYardMotorPersonsByLine = async (req: Request, res: Response) => {
    try {
        const { line_id } = req.query;

        if (!line_id || typeof line_id !== "string") {
            return res.status(400).json({ message: "Missing line_id query parameter" });
        }

        const line: Line | null = await Line.findByPk(line_id);

        if (!line) {
            return res.status(400).json({ message: "Could not find line with provided id" });
        }
...
export const getAllUsers = async (req: Request, res: Response) => {
    try {
        ...
        const users: User[] = await User.findAll({
            attributes: ['id', 'name', 'badge_number', 'username', 'line', 'role', 'key_cloak_id']
        });
